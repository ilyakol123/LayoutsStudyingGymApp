...
struct RangeSliderView: View {
    ...
    var body: some View {
        GeometryReader { reader in
            ...
            let minimumDistance =
                minimumDistance == 0
                ? 0
                : (minimumDistance / (range.upperBound - range.lowerBound))
                    * maxSliderWidth
            VStack {
                HStack {
                    Text(
                        Int(selection.lowerBound) < 3
                            ? "\(Int(selection.lowerBound)) year"
                            : "\(Int(selection.lowerBound)) years"
                    )
                    Spacer()
                                ...
                                DragGesture(minimumDistance: 0)
                                    .onChanged { value in
                                        ...
                                        var translation =
                                            ...
                                            + slider1.lastStoredOffset
                                        translation = min(
                                            max(translation, 0),
                                            slider2.offset - minimumDistance
                                        )
                            ...
                            .gesture(
                                DragGesture(minimumDistance: 0)
                                    .onChanged { value in
                                        ...
                                        var translation =
                                            value.translation.width
                                            + slider2.lastStoredOffset
                                        translation = min(
                                            max(
                                                translation,
                                                slider1.offset + minimumDistance
                                            ),
                                            maxSliderWidth
                                        )
    ...
    private func calculateNewRange(_ size: CGSize) {
        ...
        let newRangeStart = range.lowerBound.interpolated(
            towards: range.upperBound,
            amount: startProgress
        )
        let newRangeEnd = range.lowerBound.interpolated(
            towards: range.upperBound,
            amount: endProgress
        )
